...
repeatedly until no more can be found
"""

def count_adjacent_symbols(rolls):
    count = 0
    total = 0
    new_rolls = []
    ...
    for i in range(len(rolls)):
        for j in range(len(rolls[i])):
            ...
            new_rolls.append("#" if rolls[i][j] == "@" and count < 4 else rolls[i][j])
            count = 0
        new_rolls.append("\n")
    
    return new_rolls, total
...
prev_total = 0
while True:
    new_rolls, total = count_adjacent_symbols(rolls)
    prev_total += total
    rolls = ''.join(new_rolls).strip().split('\n')
